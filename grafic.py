...
class InventoryApp(tk.Tk):
    def __init__(self):
        # Inicializa la ventana principal de la aplicación y configura los widgets básicos.
        super().__init__()
    ...
    def create_widgets(self):
        # Crea los widgets gráficos: logo, barra de búsqueda, tabla, botones y etiqueta de total.
        ...
        # Botones para agregar producto y exportar CSV
        ...
        columns = ("last_update", "code", "name", "description", "unit_price",
    ...
    def refresh_table(self):
        # Recarga la lista desde la base de datos y actualiza la tabla y el total del inventario.
    ...
    def filter_table(self, event=None):
        # Filtra los productos que coinciden con el texto de búsqueda en la tabla del inventario.
        search_text = self.search_var.get().lower()
        self.tree.delete(*self.tree.get_children())
        for p in self.products_cache:
            if (search_text in str(p.code).lower() or
                search_text in p.name.lower() or
                search_text in p.description.lower() or
                search_text in p.warehouse.lower()):
                self.tree.insert(
                    "", "end", values=(
                        p.last_update,
                        str(p.code),  # ID como string
                        p.name,
                        p.description,
                        f"${p.unit_price:.2f}",
                        p.quantity,
    ...
    def add_product_window(self):
        # Abre una ventana para capturar los datos para un nuevo producto,
        # validando campos e insertando en la base de datos.
    ...
    def on_double_click(self, event):
        # Detecta doble clic en la tabla para editar o eliminar el producto correspondiente.
    ...
    def edit_product_window(self, product_values):
        # Abre una ventana para editar un producto existente, con validación y guardado.
        win = tk.Toplevel(self)
    ...
    def delete_product(self, product_values):
        # Pregunta confirmación y elimina el producto seleccionado usando su ID.
    ...
    def export_csv(self):
        # Abre diálogo para exportar la base de datos a archivo CSV.
